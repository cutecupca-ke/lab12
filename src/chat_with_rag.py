...
def get_context_with_threshold(rag_system, query, max_context_length=2000, threshold=0.3, hybrid=True):
    """
    ทำการค้นหาโดยใช้ Semantic Search และ Keyword Search
    กรองผลตาม relevance score (ถ้ามี)
    """
    if not rag_system:
        return ""
    ...
    context = ""
    if hybrid:
        semantic_results = rag_system.get_context_for_query(query, max_context_length=max_context_length)
        keyword_results = ""
        if hasattr(rag_system, "keyword_search"):
            keyword_results = rag_system.keyword_search(query, max_context_length=max_context_length)
...
def main():
    ...
    with st.sidebar:
        ...
        relevance_threshold = st.slider(
            ...
            value=0.3,
            step=0.05,
            help="Minimum relevance score to keep document results"
        )
        ...
        use_hybrid_search = st.checkbox(
            ...
            value=True,
            help="Combine keyword search and semantic search for better results"
        )
        ...
        if prompt:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Searching documents and generating response..."):
                    ...
                    #add
                    context = get_context_with_threshold(
                        st.session_state.rag_system,
                        prompt,
                        max_context_length=context_max_tokens,
