...
#include conversion_tools.py
from utils.conversion_tools import Conversion
...
def init_session_state():
    ...
    if "converter" not in st.session_state:
        st.session_state.converter = Conversion()
...
def handle_tool_calls(message_content: str):
    ...
    #currency conversion tool detection
    if "convert" in message_lower and any(curr in message_lower for curr in ["usd", "eur", "thb", "gbp", "jpy", "btc"]):
        import re
        match = re.search(r"convert\s+(\d+\.?\d*)\s+([a-zA-Z]+)\s+to\s+([a-zA-Z]+)", message_lower)
        if match:
            amount, from_curr, to_curr = match.groups()
            result = st.session_state.converter.convert(float(amount), from_curr, to_curr)
            return result, True

    # exchange rate detection
    if "exchange rate" in message_lower:
        import re
        match = re.search(r"exchange rate for\s+([a-zA-Z]+)", message_lower)
        if match:
            currency = match.group(1)
            result = st.session_state.converter.get_rate("USD", currency)
            return result, True
...
'''
#create utils/conversion_tools.py

Task: Add a new tool for currency conversion

Create a currency conversion function using a free API (e.g., exchangerate-api.com) utils/conversion_tools.py
Add detection triggers for currency-related queries
Integrate the tool into the handle_tool_calls function
